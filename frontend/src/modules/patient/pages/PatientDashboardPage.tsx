...
const specialityCards = [
  {
    ...
    hint: "Fever, cough, fatigue and everyday health concerns",
    icon: "🩺",
  },
  {
    name: "Cardiology",
    hint: "Heart health, chest discomfort and blood pressure",
    icon: "❤️",
  },
  { name: "Dermatology", hint: "Skin, hair and nail concerns", icon: "✨" },
  {
    name: "Pediatrics",
    hint: "Care for infants, children and teens",
    icon: "🧸",
  },
  {
    name: "Orthopedics",
    hint: "Bones, joints, movement and injuries",
    icon: "🦴",
  },
  {
    name: "Neurology",
    hint: "Headache, nerves and neurological care",
    icon: "🧠",
  },
];
...
export default function PatientDashboardPage() {
  ...
  const availabilityQuery = useQuery<{ slots: Slot[] }>({
    ...
    queryFn: () =>
      getDoctorAvailability(selectedDoctor!.doctorProfile.id, selectedDate),
  ...
  async function handleBook() {
    ...
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: [
          "availability",
          selectedDoctor.doctorProfile.id,
          selectedDate,
        ],
      });
      ...
      setMessage(
        getRequestErrorMessage(error, "Booking failed. Please try again."),
      );
    }
  }
  ...
  async function handleCancel(appointmentId: string) {
    try {
      ...
      setMessage(
        getRequestErrorMessage(error, "Unable to cancel this appointment."),
      );
    }
  }
  ...
  async function handleReschedule(slotStart: string) {
    ...
    try {
      ...
      setMessage(
        "Appointment rescheduled. Your calendar will update automatically if it is connected.",
      );
      ...
      setMessage(
        getRequestErrorMessage(error, "Unable to reschedule this appointment."),
      );
    }
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Find doctors, manage appointments, and stay informed.
            </p>
          </div>
        </div>
        <button
          ...
          className="rounded-xl border p-2 text-xl lg:hidden"
        >
          ☰
        </button>
        <div
          className={`${menuOpen ? "flex" : "hidden"} w-full flex-col gap-2 lg:flex lg:w-auto lg:flex-row lg:items-center`}
        >
          {[
            ...
            <Link
              key={section}
              to={`/patient/${section}`}
              className={`rounded-xl px-3 py-2 text-sm font-medium transition ${activeSection === section ? "bg-sky-600 text-white" : "text-slate-600 hover:bg-slate-100"}`}
            >
              {label}
            </Link>
          ))}
          ...
          <Link
            ...
            to="/patient/profile"
            className={`flex h-10 w-10 items-center justify-center rounded-full font-semibold ${activeSection === "profile" ? "bg-sky-700 text-white ring-2 ring-sky-200" : "bg-sky-100 text-sky-700"}`}
          >
      ...
      <div className="max-w-7xl mx-auto space-y-6">
                ...
                notificationsQuery.data.notifications.map((notification) => (
                  <p key={notification.id}>
                    ...
                    {notification.appointment
                      ? ` — ${notification.appointment.doctor.fullName}, ${new Date(notification.appointment.slotStart).toLocaleString()}`
                      : ""}
                  </p>
                ))
              ) : (
        ...
        {activeSection === "profile" && (
          <section className="bg-white rounded-2xl shadow p-6">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Keep your contact details current for appointment updates.
            </p>
            <form
            ...
            >
              ...
              <button
                ...
                className="rounded-md bg-slate-700 px-4 py-3 text-white hover:bg-slate-800 disabled:opacity-50"
              >
                Save profile
              </button>
            </form>
            ...
            >
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-3">
                ...
                <input
                  ...
                  className="rounded-md border border-gray-200 px-4 py-3"
                />
                <button className="rounded-md bg-sky-600 px-4 py-3 text-white hover:bg-sky-700">
                  Update password
                </button>
              </div>
            </form>
          ...
          >
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <input
                ...
                className="w-full rounded-md border border-gray-200 bg-white px-4 py-3 shadow-sm"
              />
              <select
                value=""
                onChange={(event) => {
                  ...
                  setSearch(value);
                  setSearchParams({ search: value, specialization: "" });
                }}
                className="w-full rounded-md border border-gray-200 bg-white px-4 py-3 shadow-sm"
              >
                <option value="">Choose a registered doctor</option>
                {doctorSuggestionsQuery.data?.doctors.map((doctor) => (
                  <option key={doctor.id} value={doctor.doctorProfile.fullName}>
                    Dr. {doctor.doctorProfile.fullName} —{" "}
              ...
              <select
                ...
                onChange={(event) => {
                  ...
                  setSearchParams({ search: "", specialization: value });
                }}
                className="w-full rounded-md border border-gray-200 bg-white px-4 py-3 shadow-sm"
              >
                ...
                {Array.from(
                  new Set(
                    doctorSuggestionsQuery.data?.doctors.map(
                      (doctor) => doctor.doctorProfile.specialization,
                    ) ?? [],
                  ),
                ).map((value) => (
            ...
            <div className="mt-6">
              ...
              {doctorsQuery.data?.doctors.length ? (
                <ul className="space-y-3">
                  {doctorsQuery.data.doctors.map((doctor) => (
                    <li key={doctor.id}>
                      ...
                      >
                        <div className="h-12 w-12 rounded-full bg-gray-200 flex items-center justify-center text-sm font-medium">
                          Dr
                        </div>
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-gray-500">
                            {doctor.doctorProfile.bio || "No bio yet"}
                          </div>
                        </div>
                ...
                <p className="text-gray-500">
                  Enter a name or specialization to search.
                </p>
              ) : null}
            </div>
          </section>
        ...
        {activeSection === "discover" && (
          <section className="overflow-hidden rounded-2xl bg-slate-900 p-6 text-white shadow">
            <div className="flex items-end justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-300">
                  Select a card to see registered doctors for that speciality.
                </p>
              </div>
              <span className="text-xs text-sky-300">
                Swipe or use the moving cards
              </span>
            </div>
            <div className="speciality-track mt-5 flex gap-4">
              {[...specialityCards, ...specialityCards].map((card, index) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    setSpecialization(card.name);
                    setSearchParams({ search: "", specialization: card.name });
                    setSelectedDoctor(null);
                  }}
                  className="w-64 shrink-0 rounded-2xl bg-white/10 p-4 text-left transition hover:bg-white/20"
                >
          ...
          >
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  See booked appointments, calendar sync status, and summary
                  progress.
                </p>
              </div>
            </div>
                      ...
                      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-medium text-emerald-700">
                            {appointment.calendarEvent?.syncStatus ??
                              "UNSYNCED"}
                          </span>
                        </div>
                      </div>
                        ...
                        <div className="rounded-2xl bg-slate-50 p-3">
                          ...
                          <p className="mt-2 text-sm text-slate-700">
                            {appointment.postVisitSummary
                              ? `${appointment.postVisitSummary.status}`
                              : "Not generated yet"}
                          </p>
                        </div>
                        <div className="rounded-2xl bg-slate-50 p-3">
                      ...
                      {reschedulingId === appointment.id && (
                        <div className="mt-4 rounded-2xl border border-sky-200 bg-sky-50 p-4">
                          ...
                          <input
                            ...
                            className="mt-3 rounded-lg border border-sky-200 px-3 py-2"
                          />
        ...
        {activeSection === "history" && (
          ...
          >
            ...
            <div className="mt-4 space-y-4">
              ...
              ) : historyQuery.data?.appointments.filter(
                  (appointment) => appointment.status !== "BOOKED",
                ).length ? (
                historyQuery.data.appointments
                        ...
                        <div className="mt-3 grid gap-3 sm:grid-cols-2">
                          <div className="rounded-xl bg-slate-50 p-3">
                            ...
                            <p className="mt-2 text-sm text-slate-700">
                              {appointment.postVisitSummary
                                ?.patientFriendlyExplanation ??
                                "Your doctor is preparing your summary."}
                            </p>
                            <p className="mt-2 text-xs text-slate-500">
                              {appointment.postVisitSummary?.status === "FAILED"
                                ? "AI summary unavailable — showing your doctor’s fallback instructions."
                                : (appointment.postVisitSummary?.status ??
                                  "PENDING")}
                            </p>
                          </div>
        ...
        {activeSection === "discover" && selectedDoctor && (
          ...
          >
            <section className="mx-auto min-h-screen w-full max-w-4xl bg-white p-5 shadow-2xl sm:my-6 sm:min-h-0 sm:rounded-2xl sm:p-6">
              <div className="mb-4 flex items-center justify-between gap-4">
                <h2 className="text-xl font-semibold">
                  Select a slot for {selectedDoctor.doctorProfile.fullName}
                </h2>
                <button
                  type="button"
                  onClick={() => {
              ...
              <div className="mb-6 rounded-2xl border border-slate-200 p-4">
                ...
                <div className="mt-2 grid grid-cols-7 gap-2">
                  ...
                  {Array.from({
                    ...
                    const onLeave = scheduleQuery.data?.leaves.some(
                      (leave) => leave.leaveDate === value,
                    );
                    const available = scheduleQuery.data?.workingHours.some(
                      (hour) => hour.dayOfWeek === date.getDay(),
                    );
                    ...
                    return (
                      <button
                        key={value}
                        type="button"
                        disabled={!available || onLeave || past}
                        onClick={() => {
                          setSelectedDate(value);
                          setSelectedSlot(null);
                        }}
                        className={`rounded-lg p-2 text-sm ${onLeave ? "bg-rose-500 text-white" : available && !past ? "bg-sky-600 text-white hover:bg-sky-700" : "bg-slate-100 text-slate-400"} ${selectedDate === value ? "ring-2 ring-slate-800" : ""}`}
                      >
                        {index + 1}
                      </button>
                    );
                ...
                <div className="mt-3 flex gap-4 text-xs text-slate-600">
                  <span>
                    <i className="mr-1 inline-block h-3 w-3 rounded bg-sky-600" />
                    Available
                  </span>
                  <span>
                    <i className="mr-1 inline-block h-3 w-3 rounded bg-rose-500" />
                    Doctor on leave
                  </span>
                </div>
              </div>
              ...
              {availabilityQuery.isError && (
                <p className="mt-4 text-rose-600">
                  {getRequestErrorMessage(
                    ...
                    "Unable to load availability. Please choose another date.",
                  )}
                </p>
              )}
              {availabilityQuery.data?.slots.length ? (
                ...
                <p className="mt-4 text-gray-500">
                  No available slots for this date. Choose a future date within
                  this doctor’s working schedule.
                </p>
              ) : (
                <p className="mt-4 text-gray-500">
